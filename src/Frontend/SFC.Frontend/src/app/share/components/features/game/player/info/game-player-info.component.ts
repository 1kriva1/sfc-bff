...
import { GamePlayerInfoConstants } from './game-player-info.constants';
import { IGamePlayerInfoModel } from './game-player-info.model';

@Component({
  selector: 'sfc-game-player-info',
  templateUrl: './game-player-info.component.html',
  styleUrls: ['./game-player-info.component.scss']
})
export class GamePlayerInfoComponent implements OnInit {
  ...
  @Input()
  stroke: number = GamePlayerInfoConstants.LOGO.STROKE;

  @Input()
  model: IGamePlayerInfoModel = {};
  ...
  @HostBinding('class')
  private get _status(): string | empty { return this.status ? `${GamePlayerInfoConstants.STATUS_CLASS_PART}-${this.status?.key}` : null };

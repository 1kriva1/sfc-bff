...
import { RequestGamePlayerInfoConstants } from './request-game-player-info.constants';
import { IRequestGamePlayerInfoModel } from './request-game-player-info.model';

@Component({
  selector: 'sfc-request-game-player-info',
  templateUrl: './request-game-player-info.component.html',
  styleUrls: ['./request-game-player-info.component.scss']
})
export class RequestGamePlayerInfoComponent implements OnInit {
  ...
  @Input()
  radius: number = RequestGamePlayerInfoConstants.LOGO.RADIUS;

  @Input()
  stroke: number = RequestGamePlayerInfoConstants.LOGO.STROKE;

  @Input()
  model: IRequestGamePlayerInfoModel = {};
  ...
  @HostBinding('class')
  private get _status(): string | empty { return this.status ? `${RequestGamePlayerInfoConstants.STATUS_CLASS_PART}-${this.status?.key}` : null };

...
import { IRequestGameTeamInfoModel } from './request-game-team-info.model';
import { RequestGameTeamInfoConstants } from './request-game-team-info.constants';

@Component({
  selector: 'sfc-request-game-team-info',
  templateUrl: './request-game-team-info.component.html',
  styleUrls: ['./request-game-team-info.component.scss']
})
export class RequestGameTeamInfoComponent implements OnInit {
  ...
  @Input()
  radius: number = RequestGameTeamInfoConstants.LOGO.RADIUS;

  @Input()
  stroke: number = RequestGameTeamInfoConstants.LOGO.STROKE;

  @Input()
  set model(value: IRequestGameTeamInfoModel) {
    this._model = value;
    this.setData();
  }
  get model(): IRequestGameTeamInfoModel {
    return this._model;
  }
  private _model: IRequestGameTeamInfoModel = {};
  ...
  private get _hostClasses(): string {
    ...
    if (this.status) {
      classes.push(`${RequestGameTeamInfoConstants.STATUS_CLASS_PART}-${this.status.key}`);
    }

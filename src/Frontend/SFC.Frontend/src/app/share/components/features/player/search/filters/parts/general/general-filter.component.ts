import { Component, OnInit } from "@angular/core";
import { FormBuilder, FormGroupDirective } from "@angular/forms";
import { Locale } from "@core/enums";
import { IEnumModel, IForm } from "@core/types";
import { StorageService } from "@core/services";
import { getWeekDays } from "@core/utils";
import { Direction } from "ngx-sfc-common";
import { IBubbleModel, ToggleType } from "ngx-sfc-inputs";
import { PlayersFilterPart } from "../../enums/players-filter-part.enum";
import { GeneralFilterLocalization } from "./general-filter.localization";
import { GeneralFilterConstants } from "./general-filter.constants";
import { IGeneralFilterModel } from "./general-filter.model";
import { BaseFilterComponent } from "../../base/base-filter.component";
import { CoreConstants } from "@core/constants/core.constants";
import { buildPlayerSearchFilterGeneralFormControls } from "./general-filter.utils";

@Component({
    selector: 'sfc-general-filter',
    templateUrl: './general-filter.component.html',
    styleUrls: ['../../base/base-filter.component.scss', './general-filter.component.scss'],
    viewProviders: [CoreConstants.CONTROL_CONTAINER_PROVIDER]
})
export class GeneralFilterComponent
    extends BaseFilterComponent
    implements OnInit {

    ToggleType = ToggleType;
    Direction = Direction;
    Localization = GeneralFilterLocalization;
    Constants = GeneralFilterConstants;

    public locale!: Locale;

    public weekDays: IBubbleModel[] = [];

    constructor(
        parent: FormGroupDirective,
        formBuilder: FormBuilder,
        private storageService: StorageService) {
        super(parent, formBuilder);
    }

    ngOnInit(): void {
        this.locale = this.storageService.get<Locale>(CoreConstants.LOCALE_KEY, Locale.English)!;
        this.weekDays = (getWeekDays() as IEnumModel<number>[]).map(d => ({ key: d.key, label: d.value }));

        if (this.build) {
            this.buildFormGroup();
        }
    }

    public generateYearsLabel(from: number, to: number): string {
        return `${GeneralFilterLocalization.INPUT.YEARS.MULTIPLE_LABEL_PART_1} ${from} - ${GeneralFilterLocalization.INPUT.YEARS.MULTIPLE_LABEL_PART_2} ${to} ${GeneralFilterLocalization.INPUT.YEARS.MULTIPLE_LABEL_PART_3}`;
    }

    private buildFormGroup(): void {
        const controls: IForm<IGeneralFilterModel> = buildPlayerSearchFilterGeneralFormControls(this.formBuilder);
        this.form.addControl(PlayersFilterPart.General, this.formBuilder.group(controls));
    }
}
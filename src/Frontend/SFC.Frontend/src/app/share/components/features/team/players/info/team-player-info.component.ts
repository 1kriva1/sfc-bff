...
import { TeamPlayerInfoConstants } from './team-player-info.constants';
import { ITeamPlayerInfoModel } from './team-player-info.model';

@Component({
  selector: 'sfc-team-player-info',
  templateUrl: './team-player-info.component.html',
  styleUrls: ['./team-player-info.component.scss']
})
export class TeamPlayerInfoComponent implements OnInit {
  ...
  @Input()
  radius: number = TeamPlayerInfoConstants.LOGO.RADIUS;

  @Input()
  stroke: number = TeamPlayerInfoConstants.LOGO.STROKE;

  @Input()
  model: ITeamPlayerInfoModel = {};
  ...
  @HostBinding('class')
  private get _status(): string | empty { return this.status ? `${TeamPlayerInfoConstants.STATUS_CLASS_PART}-${this.status?.key}` : null };
  ...
  ngOnInit(): void {
    ...
    if (isDefined(this.model.status)) {
      this.status = getEnum(this.model.status!, this.enumService.enums.teamPlayerStatuses);
    }
